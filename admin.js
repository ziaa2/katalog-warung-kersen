...
function localMode(){return !(CFG.API_BASE&&CFG.SUPABASE_URL)}
function api(path){return `${CFG.API_BASE.replace(/\/$/,"")}/${path.replace(/^\//,"")}`}
function authHeaders(extra={}){const s=JSON.parse(sessionStorage.getItem("warung-session")||"null");return {...extra,Authorization:`Bearer ${s?.access_token||""}`}}
async function load(){
  if(localMode()){
    products=JSON.parse(localStorage.getItem("warung-products")||"null") || await (await fetch("../data/products.json")).json();
    products=products.map(p=>({...p,stock_status:p.stock_status||"available"}));
...
function renderList(){
  ...
  $("#adminList").innerHTML=list.map(p=>`<div class="admin-row"><div class="mini-image">${p.image?`<img src="${esc(p.image)}">`:initials(p.name)}</div><div class="row-main"><b>${esc(p.name)}</b><small>${esc(p.brand||"")} · ${esc(p.category||"Tanpa kategori")} · ${p.barcode?esc(p.barcode):"tanpa barcode"}</small></div><div class="row-price"><b>${money(p.price)}</b><small>${p.stock_status==="low"?"Stok hampir habis":"Tersedia"}</small></div><button class="ghost" data-edit="${p.id}">Edit</button><button class="danger" data-del="${p.id}">Hapus</button></div>`).join("");
...
async function persist(){
  ...
  const p=editing||{}; const method=editing?"PATCH":"POST"; const url=editing?api(`products/${encodeURIComponent(p.id)}`):api("products");
  const body={barcode:p.barcode||null,name:p.name,brand:p.brand||null,price:Number(p.price),stock_status:p.stock_status,category:p.category||null,image:p.image||null,is_available:true};
  const r=await fetch(url,{method,headers:authHeaders({"Content-Type":"application/json"}),body:JSON.stringify(body)});if(!r.ok)throw Error("Database gagal menyimpan"); const saved=await r.json(); if(editing){const i=products.findIndex(x=>x.id===p.id);products[i]={...products[i],...saved}}else products=[...products,saved];
}
function openForm(p=null){editing=p;$("#formTitle").textContent=p?"Edit barang":"Tambah barang";$("#productId").value=p?.id||"";$("#barcode").value=p?.barcode||"";$("#name").value=p?.name||"";$("#brand").value=p?.brand||"";$("#price").value=p?.price||"";$("#stockStatus").value=p?.stock_status||"available";$("#category").value=p?.category||"";$("#image").value=p?.image||"";$("#lookupResult").innerHTML="";$("#productDialog").showModal()}
$("#addBtn").onclick=()=>openForm();$("#closeProduct").onclick=()=>$("#productDialog").close();$("#adminSearch").oninput=renderList;
$("#productForm").onsubmit=async e=>{e.preventDefault(); const p={id:editing?.id||crypto.randomUUID(),barcode:$("#barcode").value.trim(),name:$("#name").value.trim(),brand:$("#brand").value.trim(),price:Number($("#price").value),stock_status:$("#stockStatus").value,category:$("#category").value.trim()||"Lainnya",image:$("#image").value.trim()};editing=p;try{await persist();$("#productDialog").close();renderList()}catch(err){alert(err.message)}};
$("#csvInput").onchange=async e=>{const f=e.target.files[0];if(!f)return;const text=await f.text();const rows=parseCSV(text);if(!rows.length)return alert("CSV kosong.");const imported=rows.map(r=>({id:crypto.randomUUID(),barcode:r.barcode||"",name:r.nama||r.name||"",brand:r.merek||r.brand||"",price:Number(r.harga||r.price||0),stock_status:(r.status||r.stock_status||"available").toLowerCase().includes("hampir")||String(r.status||r.stock_status).toLowerCase()==="low"?"low":"available",category:r.kategori||r.category||"Lainnya",image:r.image||""})).filter(x=>x.name);if(localMode()){products=[...products,...imported];saveLocal();renderList();alert(`${imported.length} barang diimpor.`)}else alert("Untuk mode Supabase, gunakan import via endpoint/server atau masukkan CSV melalui panel yang nanti bisa kita tambah.")}
...
async function removeProduct(id){if(!confirm("Hapus barang ini?"))return;if(localMode()){products=products.filter(p=>p.id!==id);saveLocal();renderList();return}const r=await fetch(api(`products/${encodeURIComponent(id)}`),{method:"DELETE",headers:authHeaders()});if(!r.ok)return alert("Gagal menghapus");products=products.filter(p=>p.id!==id);renderList()}
